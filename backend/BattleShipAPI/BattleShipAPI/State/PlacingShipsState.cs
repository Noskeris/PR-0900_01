...
public class PlacingShipsState : IGameState
{
        ...
        IHubCallerClients clients)
    {
        ...
        await context.NotificationService.NotifyGroup(
            ...
            "PlayerDisconnected",
            $"Player {connection.Username} has disconnected. Game needs to be restarted.");
        
        var moderator = context.Db.Connections.Values
            .First(x => x.GameRoomName == connection.GameRoomName && x.IsModerator);
        
        await RestartGame(context, moderator, clients);
    }

    public async Task RestartGame(GameContext context, UserConnection connection,
        IHubCallerClients clients)
    {
